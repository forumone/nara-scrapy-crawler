...
run. spider_exception:EmptyResponseError rows (written by
UnhandledSpiderExceptionLoggingMiddleware, from
EmptyResponseGuardMiddleware raising in archive_crawler/middlewares.py)
mean a response came back with a 0-byte body despite looking otherwise
normal - confirmed live 2026-09-16, a stale/broken CloudFront edge cache
entry on trumpwhitehouse. All three mean the same thing for this check:
pushing a CSV built under those conditions risks the index-side Lambda's
mark-and-sweep reconciliation (see push.py) removing documents for URLs
that are still real content, just not fetched correctly this run.
...
_EMPTY_RESPONSE_REASON = 'spider_exception:EmptyResponseError'
...
def count_fetch_errors(csv_path):
    """Count http_5xx/network_error/empty-response rows in csv_path's
    sibling dropped-log. Returns 0 if that file does not exist."""
    ...
    with open(dropped_path, newline='', encoding='utf-8') as f:
        for row in csv.DictReader(f):
            ...
            if (
                reason == _HTTP_5XX_REASON
                or reason == _EMPTY_RESPONSE_REASON
                or reason.startswith(_NETWORK_ERROR_PREFIX)
            ):
                count += 1
    return count
...
def check_crawl_health(source_site, csv_path, threshold):
    ...
    `threshold` http_5xx/network_error/empty-response rows - signals the
    site, or the network path to it, was unreachable or returned broken
    responses for at least part of this crawl, not just that individual
    pages were excluded by content rules."""
    count = count_fetch_errors(csv_path)
    if count >= threshold:
        raise CrawlHealthError(
            f"{source_site}: {count} http_5xx/network_error/empty-response "
